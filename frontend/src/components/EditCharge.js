...
const EditCharge = ({ aircraft_data, onClose, page_name, chargeData }) => {
    ...
    useEffect(() => {
        ...
        setFormData({
            charge_type_id: chargeData.charge_type_id || '',
            category_id: chargeData.category_id || '', // ✅ add this
            service: chargeData.service || '',
            unit: chargeData.unit || '',
            rate_type: chargeData.rate_type || '',
            reference_charge_type_id: chargeData.reference_charge_type_id || '',
            count: chargeData.count || 1,
            include_aai: chargeData.include_aai ? '1' : '0',
            rate: chargeData.rate || 0,
            aai_levy: chargeData.aai_levy || 0,
            total_rate: chargeData.total_rate || 0,
            of_count: chargeData.of_count || 0,
            of_rate: chargeData.of_rate || 0,
            of_aai_levy: chargeData.of_aai_levy || 0,
            total_of_rate: chargeData.total_of_rate || 0,
        });
    }, [chargeData]);
    ...
    const handleSubmit = async () => {
        setLoading(true);
        try {
            const response = await api.put(`/charge-types/edit_charge/${chargeData.charges_id}/${page_name}`, {
                ...
                reference_charge_type_id: formData.reference_charge_type_id === '' ? null : formData.reference_charge_type_id, // ✅ Fix here
            ...
            console.error('Edit Charge Error:', error);
