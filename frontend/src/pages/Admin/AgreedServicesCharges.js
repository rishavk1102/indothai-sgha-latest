...
import { MdCheckBox, MdFlight, MdFormatListNumbered, MdOutlineSubdirectoryArrowRight, MdSubtitles } from 'react-icons/md';
...
const AgreedServicesCharges = () => {
   ...
   const parseHTMLContent = useCallback((htmlString) => {
      ...
      try {
         ...
         console.log('=== PARSE HTML: Final itemsMap ===', {
            ...
            sampleItems: Object.values(itemsMap).slice(0, 5).map(item => ({
               id: item.id,
               index: item.index,
   ...
   const parseTemplateData = useCallback((templateData) => {
      console.log('=== PARSE TEMPLATE: Starting parseTemplateData ===', {
         ...
         firstFewFields: Array.isArray(templateData) ? templateData.slice(0, 10) : templateData,
      });
      ...
      templateData.forEach((field, index) => {
         if (index < 20) { // Log first 20 fields for debugging
            console.log(`=== PARSE TEMPLATE: Field ${index} ===`, {
               type: field.type,
               value: field.value?.substring ? field.value.substring(0, 100) : field.value,
            });
                        }
                        
         if (field.type === 'heading_no') {
            const sectionNum = String(field.value);
            console.log(`=== PARSE TEMPLATE: Found heading_no "${sectionNum}" ===`, {
               index,
               currentMainSection,
            });
            ...
            if (['1', '2', '3', '4', '5', '6', '7', '8'].includes(sectionNum)) {
               if (index + 1 < templateData.length && templateData[index + 1].type === 'heading') {
                  ...
                  console.log(`=== PARSE TEMPLATE: Set currentMainSection to "${sectionNum}", mainHeading: "${mainSectionHeading}" ===`);
                  return;
                           }
                        }
            ...
            if (sectionNum.includes('.') && currentMainSection) {
               ...
               if (nextField && nextField.type === 'subheading') {
                  ...
                  console.log(`=== PARSE TEMPLATE: Set currentSection to "${sectionNum}", title: "${sectionLabel}" ===`);
                  return;
               }
            }
         }
         ...
         if (field.type === 'subheading_no' && currentMainSection) {
            ...
            if (headingNo.includes('.')) {
               ...
               titlesMap[currentSection] = sectionLabel;
               console.log(`=== PARSE TEMPLATE: Found subheading_no "${headingNo}", set currentSection, title: "${sectionLabel}" ===`);
               return;
            }
         }
         ...
         if (field.type === 'editor' && currentSection && currentMainSection) {
            if (sectionMap[currentSection]) {
               ...
               console.log(`=== PARSE TEMPLATE: Added editor content to section "${currentSection}" ===`, {
                  contentLength: field.value ? field.value.length : 0,
                  contentPreview: field.value ? field.value.substring(0, 200) : null,
               });
                  }
               }
            });
      ...
      console.log('=== PARSE TEMPLATE: Section map created ===', {
         ...
         sectionsWithContent: Object.keys(sectionMap).filter(key => sectionMap[key].editorContent),
      ...
      Object.keys(sectionMap).forEach(sectionKey => {
         ...
         console.log(`=== PARSE TEMPLATE: Processing section ${sectionKey} ===`, {
            hasEditorContent: !!sectionInfo.editorContent,
            contentLength: sectionInfo.editorContent ? sectionInfo.editorContent.length : 0,
            contentPreview: sectionInfo.editorContent ? sectionInfo.editorContent.substring(0, 300) : null,
         });
         ...
         if (sectionInfo.editorContent) {
            ...
            console.log(`=== PARSE TEMPLATE: Parsed HTML for section ${sectionKey} ===`, {
               itemsCount: Object.keys(itemsMap).length,
               itemIds: Object.keys(itemsMap).slice(0, 5),
               sampleItems: Object.values(itemsMap).slice(0, 3).map(item => ({
                  id: item.id,
                  index: item.index,
                  text: item.text?.substring(0, 50),
            ...
            Object.values(itemsMap).forEach(item => {
               if (item.index && item.text) {
                  ...
                  console.log(`=== PARSE TEMPLATE: Mapped ${fullItemNumber} (from section ${sectionKey}, item ${item.index}) -> ${item.text.substring(0, 50)}... ===`);
                     ...
                     item.subItems.forEach((subItem, subIndex) => {
                        if (subItem.index && subItem.text) {
                           ...
                           if (!isNaN(numericIndex) && numericIndex > 0) {
                              ...
                              console.log(`=== PARSE TEMPLATE: Mapped sub-item ${fullSubItemNumberLetter} AND ${fullSubItemNumberNumeric} (from section ${sectionKey}, main item ${item.index}, sub-item index ${subItem.index}) -> ${subItem.text.substring(0, 50)}... ===`);
                           } else {
                              console.warn(`=== PARSE TEMPLATE: Could not convert sub-item index "${subItem.index}" to suffix ===`);
                           }
                        }
                     });
                  }
            ...
            console.warn(`=== PARSE TEMPLATE: Section ${sectionKey} has no editor content ===`);
         }
      });
      ...
      console.log('=== PARSE TEMPLATE: Final textMap ===', {
         ...
         sampleEntries: Object.entries(textMap).slice(0, 5).map(([key, value]) => ({
            key,
   ...
   const getNextHeadingNumber = (currentFields) => {
      ...
      currentFields.forEach((field, index) => {
         if (field.type === 'heading_no' && field.value && !field.value.includes('.') &&
            index + 1 < currentFields.length && (currentFields[index + 1].type === 'heading' || currentFields[index + 1].type === 'subheading')) {
   ...
   const getNextSubheadingNumber = (currentFields) => {
      ...
      for (let i = 0; i < currentFields.length; i++) {
         if (currentFields[i].type === 'heading_no' && currentFields[i].value &&
            i + 1 < currentFields.length && currentFields[i + 1].type === 'heading') {
      ...
      if (lastHeadingIndex >= 0) {
         for (let i = lastHeadingIndex + 1; i < currentFields.length; i++) {
            if (currentFields[i].type === 'heading_no' && currentFields[i].value && String(currentFields[i].value).includes('.')) {
               ...
               if (parts.length === 2 && parseInt(parts[0], 10) === lastHeadingNumber) {
                  ...
                  if (!isNaN(subNum) && subNum > subheadingCount) subheadingCount = subNum;
               }
            }
         }
      }
   ...
   const addEditField = (type, label) => {
      ...
      if (type === 'heading') {
         ...
         newFields.push({ id: Date.now(), type: 'heading_no', label: 'Heading No.', value: nextNum.toString(), checkboxValue: [] });
      }
      if (type === 'subheading') {
         const nextNum = getNextSubheadingNumber(currentFields);
         newFields.push({ id: Date.now(), type: 'heading_no', label: 'Subheading No.', value: nextNum, checkboxValue: [] });
      }
      const newField = type === 'table'
         ? { id: Date.now() + (newFields.length * 1000), type, label, rows: [{ id: Date.now() + (newFields.length * 1000) + 1, checked: false, section: '', description: '' }] }
         : type === 'editor'
            ? { id: Date.now() + (newFields.length * 1000), type, label, value: '', checkboxValue: [], checkboxConfig: {}, commentConfig: {}, variableDefaults: {} }
      ...
      setEditFields(prev => prev.map(f => {
         if (f.id === id) {
            if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
               return { ...f, value: value.htmlValue ?? f.value ?? '', checkboxConfig: value.checkboxConfig ?? f.checkboxConfig ?? {}, commentConfig: value.commentConfig ?? f.commentConfig ?? {}, variableDefaults: value.variableDefaults ?? f.variableDefaults ?? {} };
            }
            return { ...f, value };
         }
         return f;
   ...
   const handleEditCheckboxChange = (id, option) => {
      setEditFields(prev => prev.map(f =>
         f.id === id ? { ...f, checkboxValue: f.checkboxValue?.includes(option) ? (f.checkboxValue.filter(o => o !== option)) : [...(f.checkboxValue || []), option] } : f
      ));
   };

   const addEditTableRow = (fieldId) => {
      setEditFields(prev => prev.map(f =>
         f.id === fieldId ? { ...f, rows: [...(f.rows || []), { id: Date.now(), checked: false, section: '', description: '' }] } : f
      ));
   };

   const deleteEditTableRow = (fieldId, rowId) => {
      setEditFields(prev => prev.map(f => f.id === fieldId ? { ...f, rows: (f.rows || []).filter(r => r.id !== rowId) } : f));
   };

   const handleEditTableRowChange = (fieldId, rowId, key, value) => {
      setEditFields(prev => prev.map(f =>
         f.id === fieldId ? { ...f, rows: (f.rows || []).map(r => r.id === rowId ? { ...r, [key]: value } : r) } : f
      ));
   };
   ...
   const handleSaveEdit = async () => {
      ...
      try {
         ...
         const newStatus = res.data?.data?.status ?? (editSubmission.status === 'Pending' ? 'In Progress' : editSubmission.status);
         const wasPending = editSubmission.status === 'Pending';
         setSubmissions(prev => prev.map(sub =>
            sub.submission_id === editSubmission.submission_id
               ? { ...sub, checkbox_selections: updatedSelections, status: newStatus, updated_at: new Date().toISOString() }
               : sub
         ));
         ...
         if (err.response?.status === 400 && err.response?.data?.error === 'No changes') {
   ...
   const renderEditField = (field) => {
      ...
      switch (field.type) {
         case 'textarea':
            return (
               <InputTextarea rows={3} value={field.value || ''} onChange={(e) => handleEditChange(field.id, e.target.value)} placeholder={field.label} className="w-100" style={extraStyle} />
            );
         case 'editor':
            return (
               <CustomEditor
                  value={{ htmlValue: field.value || '', checkboxConfig: field.checkboxConfig || {}, commentConfig: field.commentConfig || {}, variableDefaults: field.variableDefaults || {} }}
            ...
            return (
               <div className="d-flex gap-3" style={extraStyle}>
                  <div className="d-flex align-items-center gap-1">
                     <Checkbox inputId={`edit-yes-${field.id}`} checked={(field.checkboxValue || []).includes('Yes')} onChange={() => handleEditCheckboxChange(field.id, 'Yes')} />
                     <label htmlFor={`edit-yes-${field.id}`}>Yes</label>
                  </div>
                  <div className="d-flex align-items-center gap-1">
                     <Checkbox inputId={`edit-no-${field.id}`} checked={(field.checkboxValue || []).includes('No')} onChange={() => handleEditCheckboxChange(field.id, 'No')} />
            ...
            return (
               <div style={extraStyle}>
                  <table className="table table-bordered">
                     <thead><tr><th style={{ width: '50px' }}></th><th>Section</th><th>Description</th><th style={{ width: '50px' }}></th></tr></thead>
                     <tbody>
                        {(field.rows || []).map((row) => (
                           <tr key={row.id}>
                              <td><Checkbox checked={row.checked} onChange={(e) => handleEditTableRowChange(field.id, row.id, 'checked', e.checked)} /></td>
                              <td><InputText value={row.section || ''} onChange={(e) => handleEditTableRowChange(field.id, row.id, 'section', e.target.value)} className="w-100" /></td>
                              <td><InputTextarea value={row.description || ''} onChange={(e) => handleEditTableRowChange(field.id, row.id, 'description', e.target.value)} rows={2} className="w-100" /></td>
                              <td><Button icon="pi pi-trash" severity="danger" className="p-0 border-0" text style={{ width: '30px', height: '30px', fontSize: '12px' }} onClick={() => deleteEditTableRow(field.id, row.id)} /></td>
                           </tr>
                        ))}
                     </tbody>
                  </table>
                  <div className="d-flex justify-content-end">
                     <Button label="Add Row" icon="pi pi-plus" severity="info" className="py-1 px-2 border-0" style={{ fontSize: '14px', borderRadius: '5px' }} text onClick={() => addEditTableRow(field.id)} />
                  </div>
               </div>
            );
         default:
            return (
               <InputText value={field.value || ''} onChange={(e) => handleEditChange(field.id, e.target.value)} placeholder={field.label} className="w-100" style={extraStyle} />
            );
      }
   };
   ...
   const fetchEditHistory = async (submissionId) => {
      try {
         ...
         const res = await api.get(`/api/client/annex-a-submissions/${submissionId}/history`);
   ...
   const renderChangesSummary = (changes) => {
      ...
      return (
         <div className="changes-summary">
            {added.length > 0 && (
               <div className="mb-2">
                  <small className="fw-bold" style={{ color: '#22c55e' }}><i className="pi pi-plus-circle me-1" />Added ({added.length})</small>
                  {added.map((item, idx) => (
                     <div key={idx} className="ms-3 mt-1 p-2 rounded" style={{ backgroundColor: '#dcfce7', fontSize: '12px', borderLeft: '3px solid #22c55e' }}>
                        <div style={{ color: '#15803d', fontWeight: 600 }}>{item.fieldLabel || item.key || 'Field'}</div>
                        {item.value && typeof item.value === 'string' && item.value.length > 0 && <div className="mt-1" style={{ color: '#166534', fontSize: '11px' }}>Value: &quot;{item.value}&quot;</div>}
                     </div>
                  ))}
               </div>
            )}
            {removed.length > 0 && (
               <div className="mb-2">
                  <small className="fw-bold" style={{ color: '#ef4444' }}><i className="pi pi-minus-circle me-1" />Removed ({removed.length})</small>
                  {removed.map((item, idx) => (
                     <div key={idx} className="ms-3 mt-1 p-2 rounded" style={{ backgroundColor: '#fee2e2', fontSize: '12px', borderLeft: '3px solid #ef4444' }}>
                        <div style={{ color: '#b91c1c', fontWeight: 600, textDecoration: 'line-through' }}>{item.fieldLabel || item.key || 'Field'}</div>
                     </div>
                  ))}
               </div>
            )}
            {modified.length > 0 && (
               <div className="mb-2">
                  <small className="fw-bold" style={{ color: '#f59e0b' }}><i className="pi pi-pencil me-1" />Modified ({modified.length})</small>
                  {modified.map((item, idx) => (
                     <div key={idx} className="ms-3 mt-1 p-2 rounded" style={{ backgroundColor: '#fef3c7', fontSize: '12px', borderLeft: '3px solid #f59e0b' }}>
                        <div style={{ color: '#92400e', fontWeight: 600, marginBottom: '4px' }}>{item.fieldLabel || item.key || 'Field'}</div>
                        {item.details?.map((detail, dIdx) => (
                           <div key={dIdx} className="mt-1 ps-2" style={{ fontSize: '11px', borderLeft: '2px solid #fbbf24' }}>
                              <span style={{ color: '#78350f', fontWeight: 500 }}>{detail.property}: </span>
                              <div style={{ marginTop: '4px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                 {detail.from != null && (
                                    <div style={{ maxHeight: '200px', overflowY: 'auto', padding: '6px 8px', backgroundColor: 'rgba(185,28,28,0.06)', borderRadius: '4px', whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontSize: '11px', color: '#b91c1c', textDecoration: 'line-through' }}>
                                       {String(detail.from)}
                                    </div>
                                 )}
                                 <span style={{ color: '#808080', alignSelf: 'flex-start' }}>→</span>
                                 <div style={{ maxHeight: '200px', overflowY: 'auto', padding: '6px 8px', backgroundColor: 'rgba(21,128,61,0.06)', borderRadius: '4px', whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontSize: '11px', color: '#15803d', fontWeight: 500 }}>
                                    {String(detail.to)}
                                 </div>
                              </div>
                           </div>
   ...
   useEffect(() => {
      try {
         ...
         if (socket) {
            const handler = (data) => {
               if (historySubmission && data.submission_id === historySubmission.submission_id) fetchEditHistory(historySubmission.submission_id);
            };
   ...
   useEffect(() => {
      ...
      if (selectedSubmission.status) {
         const currentStatusOption = statusOptions.find(option => option.code === selectedSubmission.status);
   ...
   useEffect(() => {
      const fetchTemplateData = async () => {
         if (!expandedRow) {
            console.log('=== TEMPLATE DATA: Row collapsed, clearing itemTextMap ===');
            setItemTextMap({});
            setSectionTitles({});
            return;
      }
         ...
         console.log('=== TEMPLATE DATA: Fetching template data for expanded row ===', expandedRow);
         ...
         try {
            ...
            if (response.data?.data?.content) {
               ...
               console.log('=== TEMPLATE DATA: Raw content ===', {
                  ...
                  preview: typeof content === 'string' ? content.substring(0, 500) : content,
               });
               ...
               try {
                  ...
                  console.error('=== TEMPLATE DATA: Error parsing content ===', parseError);
                  setItemTextMap({});
                  return;
               }
               ...
               console.log('=== TEMPLATE DATA: Parsed content ===', {
                  ...
                  firstFewItems: Array.isArray(parsedContent) ? parsedContent.slice(0, 5) : parsedContent,
                  fieldTypes: Array.isArray(parsedContent) ? parsedContent.slice(0, 20).map(f => f?.type).filter(Boolean) : [],
               });
            ...
            console.error("=== TEMPLATE DATA: Error fetching template data ===", error);
            setItemTextMap({});
         } finally {
   ...
   const getItemText = (itemNumber) => {
      ...
      if (itemTextMap[itemNumber]) {
         console.log(`=== GET ITEM TEXT: Found direct match for "${itemNumber}" ===`, itemTextMap[itemNumber].substring(0, 50));
      ...
      if (itemTextMap[cleanNumber]) {
         console.log(`=== GET ITEM TEXT: Found match after removing "-main" for "${itemNumber}" ===`, itemTextMap[cleanNumber].substring(0, 50));
      ...
      if (letterSuffixMatch) {
         ...
         console.log(`=== GET ITEM TEXT: Letter suffix match for "${itemNumber}", parent: "${parentNumber}", letter: "${letter}" ===`);
         ...
         if (itemTextMap[parentNumber]) {
            ...
            console.log(`=== GET ITEM TEXT: Found parent "${parentNumber}", returning parent text with letter ===`);
            return `${itemTextMap[parentNumber]} (${letter})`;
         } else {
            console.log(`=== GET ITEM TEXT: Parent "${parentNumber}" not found in itemTextMap ===`);
         }
      }
      ...
      if (numericSuffixMatch) {
         ...
         console.log(`=== GET ITEM TEXT: Numeric suffix match for "${itemNumber}", parent: "${parentNumber}", suffix: "${numericSuffix}" ===`);
         ...
         if (!isNaN(letterIndex) && letterIndex > 0) {
            ...
            if (itemTextMap[letterKey]) {
               console.log(`=== GET ITEM TEXT: Found match using letter key "${letterKey}" ===`);
         ...
         if (itemTextMap[parentNumber]) {
            console.log(`=== GET ITEM TEXT: Found parent "${parentNumber}", returning sub-item indicator ===`);
      ...
      if (parts.length >= 3) {
         ...
         if (itemTextMap[baseNumber]) {
            console.log(`=== GET ITEM TEXT: Found partial match for "${itemNumber}" using base "${baseNumber}" ===`, itemTextMap[baseNumber].substring(0, 50));
   ...
   const handleStatusUpdate = async () => {
      ...
      if (dialogTemplateVariables.length > 0) {
         ...
         console.log('[Status Update] Skipping Company\'s Pricing validation (no template variables)');
      }
                                       ...
                                       {role !== 'Client' && (
                                          <>
                                             ...
                                             <Button
                                                ...
                                                text
                                                severity="secondary"
                                                style={{ fontSize: '10px', width: '28px', flexShrink: 0 }}
                                                tooltip="Edit history"
                                                tooltipOptions={{ position: 'top', hideDelay: 0, showDelay: 200 }}
                                    ...
                                    <div className="p-3">
                                       ...
                                       <div style={{ overflowX: 'auto', overflowY: 'visible', width: '100%' }}>
                                          ...
                                          <div className="commentbody">
                                             <ul>
                                                <li>
                                                   ...
                                                   {submission.last_employee_edit_at && (
                                                      <span className="d-block mt-1">Last edited (Employee): {formatDateTime(submission.last_employee_edit_at)}</span>
                                                   )}
                                          ...
                                          <div className="adsbody">
                                             <ul>
                                                <li>
                                                   <p className="mb-0"><b>Current Status: {submission.status}</b></p>
   ...
   return (
      <>
         ...
         <Dialog
            ...
            modal={true}
         >
            <h6>{selectedSubmission?.client_name || 'Update Status & Company\'s Pricing'}</h6>
            <hr />
            ...
            <div className="mb-4">
               <label className="mb-2 d-block fw-semibold">Effective Start Date</label>
               <Calendar
            ...
            {(loadingDialogVariables || dialogTemplateVariables.length > 0) && (
               <div className="mb-4">
                  <label className="mb-2 d-block fw-semibold">Company's Pricing</label>
                        ...
                        {dialogTemplateVariables.map((varName) => {
                           ...
                           return (
                              <div key={varName} className="mb-3">
                                 ...
                                 <InputText
                                    value={typeof varValue === 'object' ? JSON.stringify(varValue) : String(varValue || '')}
                                    onChange={(e) => {
         ...
         {role !== 'Client' && (
            <Dialog
               ...
               header={
                  <div className="d-flex align-items-center gap-2">
                     <i className="pi pi-file-edit" style={{ fontSize: '1.1rem', color: 'rgb(146 74 151)' }}></i>
                     <span>Edit agreement {editSubmission ? `- ${editSubmission.client_name}` : ''}</span>
                  </div>
               }
               onHide={() => {
                  setEditDialogVisible(false);
                  ...
                  <Row>
                        ...
                        <div style={{ maxHeight: 'calc(80vh - 120px)', overflowY: 'auto', overflowX: 'hidden' }}>
                           <Card className="mb-3 border-0">
                              <Card.Body>
                                 {editFields.length === 0 ? (
                                    <div className="text-center py-5">
                                       <i className="pi pi-inbox" style={{ fontSize: '3rem', color: '#bdbdbd' }}></i>
                                       <p className="mt-3 mb-0 text-muted">No template fields yet. Use the sidebar to add items or load from template.</p>
                                    </div>
                                 ) : (
                                    editFields.map((field, index) => {
                                       const isHeadingNo = field.type === 'heading_no' && index + 1 < editFields.length && (editFields[index + 1].type === 'heading' || editFields[index + 1].type === 'subheading');
                                       if (isHeadingNo) {
                                             ...
                                             <div key={field.id} className="mb-3">
                                                <div className="d-flex align-items-center justify-content-between mb-2">
                                                   <div className="d-flex gap-3 align-items-center">
                                                      <label className="fw-bold mb-0" style={{ minWidth: '100px' }}>{field.label}:</label>
                                                      <label className="fw-bold mb-0" style={{ flex: 1 }}>{nextField.label}:</label>
                                                   </div>
                                                   <Button icon="pi pi-times" severity="danger" className="p-0 border-0" text style={{ width: '30px', height: '30px', fontSize: '12px' }} onClick={() => { deleteEditField(field.id); deleteEditField(nextField.id); }} />
                                                </div>
                                                <div className="d-flex gap-2 align-items-center">
                                                   <div style={{ width: '150px', flexShrink: 0 }}><InputText value={field.value} disabled className="w-100" /></div>
                                                   <div style={{ flex: 1 }}>{renderEditField(nextField)}</div>
                                                </div>
                                             </div>
                                          );
                                       }
                                       const wasRendered = index > 0 && editFields[index - 1].type === 'heading_no' && (field.type === 'heading' || field.type === 'subheading');
                                       if (wasRendered) return null;
                                       return (
                                          <div key={field.id} className="mb-3">
                                             <label className="fw-bold w-100 mt-2 d-flex align-items-center justify-content-between">
                                                {field.label}
                                                <Button icon="pi pi-times" severity="danger" className="p-0 border-0" text style={{ width: '30px', height: '30px', fontSize: '12px' }} onClick={() => deleteEditField(field.id)} />
                                             </label>
                                             <div className="w-100">{renderEditField(field)}</div>
                                          </div>
                                       );
                                    })
                                 )}
                     ...
                     <Col md={3} lg={2}>
                        <div className="sticky-top" style={{ top: '0px' }}>
                           <ul className="ItemList" style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                              <li onClick={() => addEditField('heading', 'Heading')} style={{ cursor: 'pointer', padding: '8px 12px', border: '1px solid #eee', borderRadius: '5px', marginBottom: '4px', fontSize: '13px', display: 'flex', alignItems: 'center' }}><FaHeading className="me-2" />Heading</li>
                              <li onClick={() => addEditField('heading_no', 'Heading No.')} style={{ cursor: 'pointer', padding: '8px 12px', border: '1px solid #eee', borderRadius: '5px', marginBottom: '4px', fontSize: '13px', display: 'flex', alignItems: 'center' }}><MdFormatListNumbered className="me-2" />Heading No.</li>
                              <li onClick={() => addEditField('subheading', 'Subheading')} style={{ cursor: 'pointer', padding: '8px 12px', border: '1px solid #eee', borderRadius: '5px', marginBottom: '4px', fontSize: '13px', display: 'flex', alignItems: 'center' }}><MdSubtitles className="me-2" />Sub-heading</li>
                              <li onClick={() => addEditField('sub_child', 'Sub Child')} style={{ cursor: 'pointer', padding: '8px 12px', border: '1px solid #eee', borderRadius: '5px', marginBottom: '4px', fontSize: '13px', display: 'flex', alignItems: 'center' }}><MdOutlineSubdirectoryArrowRight className="me-2" />Sub-child</li>
                              <li onClick={() => addEditField('input', 'Input Box')} style={{ cursor: 'pointer', padding: '8px 12px', border: '1px solid #eee', borderRadius: '5px', marginBottom: '4px', fontSize: '13px', display: 'flex', alignItems: 'center' }}><AiOutlineEdit className="me-2" />Input Box</li>
                              <li onClick={() => addEditField('editor', 'Text Editor')} style={{ cursor: 'pointer', padding: '8px 12px', border: '1px solid #eee', borderRadius: '5px', marginBottom: '4px', fontSize: '13px', display: 'flex', alignItems: 'center' }}><AiOutlineForm className="me-2" />Text editor</li>
                              <li onClick={() => addEditField('textarea', 'Text Area')} style={{ cursor: 'pointer', padding: '8px 12px', border: '1px solid #eee', borderRadius: '5px', marginBottom: '4px', fontSize: '13px', display: 'flex', alignItems: 'center' }}><BiText className="me-2" />Text Area</li>
                              <li onClick={() => addEditField('table', 'Table Field')} style={{ cursor: 'pointer', padding: '8px 12px', border: '1px solid #eee', borderRadius: '5px', marginBottom: '4px', fontSize: '13px', display: 'flex', alignItems: 'center' }}><LuTable className="me-2" />Table</li>
                              <li onClick={() => addEditField('checkbox', 'Checkbox')} style={{ cursor: 'pointer', padding: '8px 12px', border: '1px solid #eee', borderRadius: '5px', marginBottom: '4px', fontSize: '13px', display: 'flex', alignItems: 'center' }}><MdCheckBox className="me-2" />Checkbox</li>
                           </ul>
                           <div className="mt-3">
                              <label className="fw-bold mb-1" style={{ fontSize: '13px' }}>Edit note (optional)</label>
                              <InputTextarea value={editNote} onChange={e => setEditNote(e.target.value)} rows={2} className="w-100" placeholder="Describe what you changed..." style={{ fontSize: '12px' }} />
                           </div>
                           <div className="mt-3">
                              <Button label="Cancel" severity="secondary" style={{ fontSize: '14px', borderRadius: '5px' }} className="py-1 w-100 d-block mb-2" outlined onClick={() => { setEditDialogVisible(false); setEditSubmission(null); setEditFields([]); setEditNote(''); initialEditFieldsRef.current = null; submissionHadTemplateFieldsRef.current = false; }} />
                              <Button label="Save Changes" icon="pi pi-check" severity="success" style={{ fontSize: '14px', borderRadius: '5px' }} className="py-1 w-100 d-block" onClick={handleSaveEdit} loading={editSaving} disabled={editSaving} />
                           </div>
                        </div>
                     </Col>
                  </Row>
            ...
            <Dialog
               ...
               header={
                  <div className="d-flex align-items-center gap-2">
                     ...
                     <span>Edit History {historySubmission ? `- ${historySubmission.client_name}` : ''}</span>
                  </div>
               }
               onHide={() => { setHistoryDialogVisible(false); setHistorySubmission(null); setEditHistory([]); }}
            >
               {historyLoading ? (
                  <div className="text-center py-4">
                     <i className="pi pi-spin pi-spinner" style={{ fontSize: '1.5rem', color: '#808080' }}></i>
                     <p className="mt-2 mb-0" style={{ color: '#808080' }}>Loading edit history...</p>
                  </div>
               ) : editHistory.length === 0 ? (
                  <div className="text-center py-4">
                     <i className="pi pi-clock" style={{ fontSize: '2.5rem', color: '#bdbdbd' }}></i>
                     <p className="mt-2 mb-0" style={{ color: '#808080' }}>No edit history yet.</p>
                     <small className="text-muted">Changes will appear here when the agreement is edited.</small>
                  </div>
               ) : (
                  <div style={{ maxHeight: '65vh', overflowY: 'auto' }}>
                     {editHistory.map((item, idx) => (
                        <div key={item.edit_id} className="p-3 mb-3 rounded" style={{ border: '1px solid', borderColor: idx === 0 ? 'rgba(146, 74, 151, 0.3)' : '#e5e7eb', backgroundColor: idx === 0 ? '#f3f0ff' : '#fff' }}>
                           <div className="d-flex justify-content-between align-items-start mb-2">
                              <div className="d-flex align-items-center gap-2">
                                 <Avatar label={item.editor_name?.charAt(0)?.toUpperCase()} shape="circle" style={{ backgroundColor: item.editor_type === 'Client' ? '#ff9832' : 'rgb(146 74 151)', color: '#fff', width: '32px', height: '32px', minWidth: '32px', fontSize: '14px' }} />
                                 <div>
                                    <div style={{ fontSize: '13px', fontWeight: 600 }}>{item.editor_name} <Badge className="ms-2" style={{ fontSize: '10px', fontWeight: 500, backgroundColor: item.editor_type === 'Client' ? '#ff9832' : 'rgb(146 74 151)', color: '#fff', border: 'none' }}>{item.editor_type}</Badge></div>
                                    {item.edit_note && <p className="mb-0 mt-1" style={{ fontSize: '12px', color: '#6b7280', fontStyle: 'italic' }}>&quot;{item.edit_note}&quot;</p>}
                                 </div>
                              </div>
                              <div className="text-end">
                                 <div style={{ fontSize: '11px', color: '#808080' }}>{formatCommentTime(item.created_at)}</div>
                                 <Badge bg="light" text="dark" className="mt-1" style={{ fontSize: '10px', border: '1px solid #e5e7eb' }}>v{item.version}</Badge>
                              </div>
                           </div>
               ...
               {commentsLoading ? (
                  ...
                  <div className="text-center py-4">
                     ...
                     <p className="mt-2 mb-0" style={{ color: '#808080' }}>
                        No comments yet. {senderType === 'Client' ? 'Start a conversation!' : 'Waiting for client comments.'}
                     </p>
                  </div>
               ) : (
                  [...comments]
