...
function compareTemplateFields(prevFields, newFields) {
  ...
  if (prevArr.length === 0 && newArr.length > 0) {
    const modifiedCount = newArr.filter((f) => {
      ...
      const hasCheckbox = Array.isArray(f.checkboxValue) && f.checkboxValue.length > 0;
      const hasRows = f.type === 'table' && Array.isArray(f.rows) && f.rows.length > 0;
      return hasValue || hasCheckbox || hasRows;
    }).length;
    changes.modified.push({
      fieldLabel: 'Agreement content',
      fieldType: 'data',
      details: [{ property: 'summary', from: '—', to: `Agreement content updated (${modifiedCount} field(s) in this version)` }],
    });
    return changes;
  }
  ...
  for (const [id, newField] of newMap) {
    if (prevMap.has(id)) {
      ...
      if (prevField.label !== newField.label) {
        fieldChanges.push({ property: 'label', from: prevField.label || '', to: newField.label || '' });
      }
      if (JSON.stringify(prevField.checkboxValue) !== JSON.stringify(newField.checkboxValue)) {
        fieldChanges.push({
 ...
 */
function generateChangesSummary(previousData, newData, previousTemplateFieldsOverride = null) {
  const prev = previousData || {};
  const next = newData || {};
  const changes = { added: [], removed: [], modified: [] };

  const prevTemplateFields = previousTemplateFieldsOverride ?? prev._templateFields;
  if (prevTemplateFields || next._templateFields) {
    const fieldChanges = compareTemplateFields(prevTemplateFields, next._templateFields);
  ...
  for (const key of allKeys) {
    ...
    if ((prev._templateFields || next._templateFields) && !topLevelKeysToReport.has(key)) continue;
    const prevVal = prev[key];
    const nextVal = next[key];

    if (prevVal === undefined && nextVal !== undefined) {
      if (key === 'serviceTypes' && typeof nextVal === 'object') {
        const activeTypes = Object.entries(nextVal).filter(([, v]) => v).map(([k]) => k).join(', ') || 'none';
        changes.added.push({ fieldLabel: `Service Types: ${activeTypes}`, fieldType: 'serviceTypes', value: activeTypes });
      } else {
        changes.added.push({ fieldLabel: key, fieldType: 'data', value: typeof nextVal === 'object' ? JSON.stringify(nextVal) : nextVal });
      }
    } else if (prevVal !== undefined && nextVal === undefined) {
      changes.removed.push({ fieldLabel: key, fieldType: 'data', value: typeof prevVal === 'object' ? JSON.stringify(prevVal) : prevVal });
    } else if (JSON.stringify(prevVal) !== JSON.stringify(nextVal)) {
      if (key === 'serviceTypes' && typeof prevVal === 'object' && typeof nextVal === 'object') {
        const prevTypes = Object.entries(prevVal).filter(([, v]) => v).map(([k]) => k).join(', ') || 'none';
        const nextTypes = Object.entries(nextVal).filter(([, v]) => v).map(([k]) => k).join(', ') || 'none';
        changes.modified.push({
        ...
        changes.modified.push({
          fieldLabel: key,
          fieldType: 'data',
          details: [{ property: 'value', from: truncate(JSON.stringify(prevVal), 100), to: truncate(JSON.stringify(nextVal), 100) }],
        });
      }
    }
  }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log('4. checkbox_selections type:', typeof req.body.checkbox_selections);
      console.log('5. checkbox_selections keys:', req.body.checkbox_selections ? Object.keys(req.body.checkbox_selections) : 'null');
      console.log('6. checkbox_selections.serviceTypes:', req.body.checkbox_selections?.serviceTypes);
      console.log('7. Full checkbox_selections:', JSON.stringify(req.body.checkbox_selections, null, 2));
      console.log('=== END BACKEND DEBUG ===');
      
      // Get client_registration_id from request body (since auth is disabled)
      const { client_registration_id, agreement_year, checkbox_selections, form_details } = req.body;
      ...
      if (typeof checkbox_selections !== 'object' || Array.isArray(checkbox_selections)) {
        return res.status(400).json({
      ...
      const year = typeof agreement_year === 'number'
        ? agreement_year
      ...
      const submission = await ClientAnnexASubmission.create({
        ...
        agreement_year: year,
        checkbox_selections,
        form_details: form_details && typeof form_details === 'object' ? form_details : null,
        submission_timestamp: new Date(),
      });

      console.log(`✅ Annex A submission saved for client ${client_registration_id}, year ${year}`);
 ...
 */
router.get(
  '/annex-a-submissions-list',
  async (req, res) => {
    try {
      const { status, year, sortBy = 'DESC', limit, offset, client_registration_id } = req.query;
      ...
      if (status) {
        if (client_registration_id) {
          ...
          const validClientStatuses = ['Pending', 'In Progress', 'Approved', 'Rejected'];
          ...
          const validStatuses = ['Pending', 'In Progress', 'Completed', 'Suspended', 'Cancelled', 'Expired'];
      ...
      const formattedSubmissions = submissions.map((submission) => {
        ...
        const lastEmployeeEdit = sortedHistory.find((e) => e.editor_type === 'Employee');
        const last_employee_edit_at = lastEmployeeEdit ? lastEmployeeEdit.created_at : null;
      ...
      if (client_registration_id) {
        ...
        const pendingOnlyCount = await ClientAnnexASubmission.count({
          where: { ...countWhereClause, client_status: 'Pending', status: 'Pending' },
        });
        ...
        const statusCounts = await ClientAnnexASubmission.findAll({
          ...
          attributes: ['status', [sequelize.fn('COUNT', sequelize.col('submission_id')), 'count']],
          group: ['status'],
          raw: true,
        });
        counts = {
 ...
 */
router.put(
  '/annex-a-submissions/:submission_id/status',
  async (req, res) => {
    try {
      ...
      if (client_status !== undefined && client_status !== null) {
        ...
        if (!validClientStatuses.includes(client_status)) {
          return res.status(400).json({
            message: 'Invalid client_status. Must be one of: Pending, Approved, Rejected',
            error: 'Validation error',
          });
        }
        ...
        if (!validStatuses.includes(status)) {
          return res.status(400).json({
            message: 'Invalid status. Must be one of: Pending, In Progress, Completed, Suspended, Cancelled, Expired',
            error: 'Validation error',
          });
        }
      } else {
      ...
      const updatedField = client_status !== undefined ? `client_status: ${client_status}` : `status: ${status}`;
      console.log(`✅ ${updatedField} updated for submission ${submission_id}${effectiveFromDate ? `, effective_from: ${effectiveFromDate.toISOString()}` : ''}${effectiveToDate ? `, effective_to: ${effectiveToDate.toISOString()}` : ''}`);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Get Annex B template for the agreement year
      ...
      console.log('🔍 Extracting variables from template for submission:', submissionIdInt);
      console.log('📋 Template content type:', Array.isArray(templateContent) ? 'Array' : typeof templateContent);
      ...
      const extractVariablesFromText = (text) => {
        ...
        while ((match = variableRegex.exec(text)) !== null) {
          ...
          if (varName && !reservedVariables.includes(varName) && !found.includes(varName)) {
            found.push(varName);
          }
        }
        return found;
      ...
      if (Array.isArray(templateContent)) {
        templateContent.forEach((field, index) => {
          ...
          if (field.type === 'editor' && field.value) {
            const htmlContent = typeof field.value === 'string' ? field.value : '';
            console.log(`  📝 Checking editor field ${index}, content length: ${htmlContent.length}`);
            ...
            foundVars.forEach(varName => {
              if (!variables.includes(varName)) {
                ...
                console.log(`  ✅ Found variable in editor field ${index}: ${varName}`);
              }
            });
          }
          ...
          if (field.variableDefaults && typeof field.variableDefaults === 'object') {
            Object.keys(field.variableDefaults).forEach(varName => {
              if (varName && !reservedVariables.includes(varName) && !variables.includes(varName)) {
                variables.push(varName);
                console.log(`  ✅ Found variable in variableDefaults: ${varName}`);
              }
            });
          }
        });
      ...
      console.log(`📊 Total variables found (excluding reserved): ${variables.length}`, variables);
...
router.post(
  ...
  async (req, res) => {
    try {
      const { submission_id } = req.params;
      const { sender_type, sender_id, sender_name, message, parent_comment_id } = req.body;

      if (!sender_type || !sender_id || !sender_name || !message) {
        return res.status(400).json({
          message: 'sender_type, sender_id, sender_name, and message are required',
          error: 'Validation error',
        });
      }
      ...
      if (parent_comment_id) {
        ...
        if (!parentComment) {
          return res.status(404).json({ message: 'Parent comment not found for this submission' });
        }
      }
 ...
 */
router.put(
  '/annex-a-submissions/:submission_id/edit',
  async (req, res) => {
    const transaction = await sequelize.transaction();
    try {
      const { submission_id } = req.params;
      const { checkbox_selections, editor_type, editor_id, editor_name, edit_note, previous_template_fields: previousTemplateFieldsOverride } = req.body;

      if (!checkbox_selections || !editor_type || !editor_id || !editor_name) {
        await transaction.rollback();
        return res.status(400).json({
          message: 'checkbox_selections, editor_type, editor_id, and editor_name are required',
          error: 'Validation error',
        });
      }
      ...
      const submission = await ClientAnnexASubmission.findByPk(submission_id, { transaction });
      if (!submission) {
        await transaction.rollback();
        return res.status(404).json({ message: 'Submission not found' });
      }
      ...
      const changesSummary = generateChangesSummary(previousData, checkbox_selections, previousTemplateFieldsOverride);
      ...
      console.log(`Submission ${submission_id} edited by ${editor_name} (v${nextVersion})`);
 ...
 */
router.get(
  '/annex-a-submissions/:submission_id/history',
  async (req, res) => {
    try {
      const { submission_id } = req.params;
 ...
 */
router.get(
  '/annex-a-submissions/:submission_id/details',
  async (req, res) => {
    try {
      const { submission_id } = req.params;
        ...
        include: [
          {
            model: Client_Registration,
            as: 'clientRegistration',
            attributes: ['client_registration_id', 'name', 'email', 'contact_person', 'phone'],
          },
        ],
      });
