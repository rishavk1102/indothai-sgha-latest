 ...
 */
export const generateSubmissionPDF = async (submission, itemTextMap = {}, sectionTitles = {}, options = {}) => {
  ...
  try {
    ...
    if (Object.keys(itemTextMap).length === 0 || Object.keys(sectionTitles).length === 0) {
      const templateData = await fetchAnnexATemplateData(submission.agreement_year || 2025);
    ...
    console.log('[PDF] ItemTextMap loaded:', {
      ...
      sampleValues: Object.keys(finalItemTextMap).slice(0, 3).map(k => ({ key: k, text: finalItemTextMap[k]?.substring(0, 50) }))
    });
    ...
    const margin = 15;
    const footerPadding = 10; // Padding from bottom edge for footer text baseline
    ...
    const bottomMargin = footerTotalHeight; // Bottom margin = footer space (content should not go below this)
    ...
    // Returns the new Y position after the text (so caller can update yPos)
    const addText = (text, x, y, options = {}) => {
      ...
      doc.text(lines, x, actualY, { align });
      return actualY + totalHeight; // Return the Y position after the text
    };
    ...
    const splitAnnexBContentByVariables = (html) => {
      if (!html || typeof html !== 'string') return [{ content: html || '', isTable: false }];
      const result = [];
      ...
      if (result.length === 0) result.push({ content: html.trim(), isTable: false });
      return result;
    };

    // Draw a simple table and return new y position
    const drawSimpleTable = (startX, startY, headers, rows, colWidths, opts = {}) => {
      ...
      headers.forEach((h, i) => {
        ...
        if (i < colWidths.length - 1) {
          doc.line(x + colWidths[i], startY, x + colWidths[i], currentY + rowHeight);
        }
        x += colWidths[i];
      });
      doc.line(startX + tableWidth, startY, startX + tableWidth, currentY + rowHeight);
      currentY += rowHeight;
      doc.line(startX, currentY, startX + tableWidth, currentY);
      ...
      rows.forEach((row) => {
        ...
        const cells = Array.isArray(row) ? row : headers.map((_, i) => row[headers[i]] ?? '-');
        x = startX;
        cells.forEach((cell, i) => {
          doc.text(String(cell ?? '-').substring(0, 32), x + 2, currentY + (rowHeight * 0.5));
          if (i < colWidths.length - 1) doc.line(x + colWidths[i], currentY, x + colWidths[i], currentY + rowHeight);
          x += colWidths[i];
        });
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('SGHA Agreement Document', pageWidth / 2, yPos, { align: 'center' });
    yPos += 10;
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    yPos = addText(`Client Name: ${submission.client_name || 'N/A'}`, margin, yPos, { fontSize: 10 });
    yPos = addText(`Contact Name: ${submission.contact_name || 'N/A'}`, margin, yPos, { fontSize: 10 });
    yPos = addText(`Contact Email: ${submission.contact_email || 'N/A'}`, margin, yPos, { fontSize: 10 });
    yPos = addText(`Contact Phone: ${submission.contact_phone || 'N/A'}`, margin, yPos, { fontSize: 10 });
    yPos = addText(`Service Type: ${submission.service_type || 'N/A'}`, margin, yPos, { fontSize: 10 });
    yPos = addText(`Agreement Year: ${submission.agreement_year || 'N/A'}`, margin, yPos, { fontSize: 10 });
    yPos = addText(`Effective From: ${submission.effective_from ? new Date(submission.effective_from).toLocaleDateString() : 'N/A'}`, margin, yPos, { fontSize: 10 });
    yPos = addText(`Effective To: ${submission.effective_to ? new Date(submission.effective_to).toLocaleDateString() : 'N/A'}`, margin, yPos, { fontSize: 10 });
    yPos = addText(`Status: ${submission.status || 'N/A'}`, margin, yPos, { fontSize: 10 });
    yPos += 10;
    ...
    doc.setFont('helvetica', 'normal');
    yPos = addText('hereinafter referred to as the \'Carrier\' or the \'Handling Company\' as the case may be,', margin, yPos, { fontSize: 10 });
    yPos += 5;
    ...
    doc.setFont('helvetica', 'normal');
    const line4Text = 'and:';
    const handlingCompanyName = clientDetails?.name || submission.client_name || 'INDOTHAI KOLKATA PRIVATE LIMITED';
    ...
    doc.setFont('helvetica', 'normal');
    yPos = addText('hereinafter referred to as the \'Handling Company\' or the \'Carrier\', as the case may be, the Carrier and/or the Handling Company may hereinafter be referred to as the "Party(ies)" Whereby all the parties agree as follows:', margin, yPos, { fontSize: 10 });
    yPos += 10;

    try {
      // Fetch Main Agreement template
      const mainAgreementResponse = await api.get(
        `/sgha_template_content/get/${submission.agreement_year || 2025}/Main Agreement/Section Template`
      );
        ...
        parsedContent.forEach((field, index) => {
          if (field.type === 'heading_no') {
            ...
            if (!sectionNum.includes('.') && ['1', '2', '3', '4', '5', '6', '7', '8', '9'].includes(sectionNum)) {
              if (index + 1 < parsedContent.length && parsedContent[index + 1].type === 'heading') {
                currentMainSection = sectionNum;
                currentArticle = {
                  articleNumber: sectionNum,
                  articleTitle: parsedContent[index + 1].value,
                  sections: []
                };
            ...
            if (sectionNum.includes('.') && currentMainSection) {
              const nextField = index + 1 < parsedContent.length ? parsedContent[index + 1] : null;
              if (nextField && nextField.type === 'subheading') {
                currentSection = {
        ...
        sections.forEach((article) => {
          ...
          article.sections.forEach((section) => {
            ...
            if (section.sectionNumber && section.sectionTitle) {
              doc.setFontSize(11);
              doc.setFont('helvetica', 'bold');
              doc.text(`${section.sectionNumber} ${section.sectionTitle}`, margin, yPos);
              yPos += 7;
            }
            ...
            if (section.content) {
              ...
              tempDiv.innerHTML = DOMPurify.sanitize(section.content, {
                ALLOWED_TAGS: ['p', 'ul', 'ol', 'li', 'strong', 'em', 'b', 'i', 'br', 'div', 'span'],
                ALLOWED_ATTR: []
              });
              ...
              const processNode = (node, indent = 0, isBold = false) => {
                ...
                if (node.nodeType === 3) { // Text node
                  ...
                  if (text) {
                    ...
                    yPos = addText(text, margin + indent, yPos, { fontSize: 10 });
                  }
                  return;
                }
                ...
                if (node.nodeType === 1) { // Element node
                  ...
                  const shouldBeBold = isBold || tagName === 'strong' || tagName === 'b';
                  ...
                  if (tagName === 'p') {
                    ...
                    const pText = node.textContent?.trim() || node.innerText?.trim();
                    if (pText) {
                      doc.setFont('helvetica', 'normal');
                      yPos = addText(pText, margin + indent, yPos, { fontSize: 10 });
                      ...
                      const processListItem = (liNode) => {
                        ...
                        Array.from(liNode.childNodes).forEach(child => {
                          if (child.nodeType === 3) {
                            ...
                            if (text) {
                              doc.setFont('helvetica', 'normal');
                              yPos = addText(`• ${text}`, margin + indent + 5, yPos, { fontSize: 10 });
                              hasContent = true;
                            }
                            ...
                            if (childTag === 'strong' || childTag === 'b') {
                              ...
                              if (boldText) {
                                yPos = addText(`• ${boldText}`, margin + indent + 5, yPos, { fontSize: 10 });
                                hasContent = true;
                              }
                              doc.setFont('helvetica', 'normal');
                      ...
                      const processListItem = (liNode) => {
                        ...
                        Array.from(liNode.childNodes).forEach(child => {
                          if (child.nodeType === 3) {
                            ...
                            if (text) {
                              doc.setFont('helvetica', 'normal');
                              yPos = addText(`${index + 1}. ${text}`, margin + indent + 5, yPos, { fontSize: 10 });
                              hasContent = true;
                            }
                          } else if (child.nodeType === 1) {
                            const childTag = child.tagName?.toLowerCase();
                            if (childTag === 'strong' || childTag === 'b') {
                              ...
                              const boldText = child.textContent?.trim();
                              if (boldText) {
                                yPos = addText(`${index + 1}. ${boldText}`, margin + indent + 5, yPos, { fontSize: 10 });
                                hasContent = true;
                              }
                              doc.setFont('helvetica', 'normal');
                            } else if (childTag === 'ul' || childTag === 'ol') {
                    ...
                    if (boldText) {
                      yPos = addText(boldText, margin + indent, yPos, { fontSize: 10 });
                    }
                    doc.setFont('helvetica', 'normal');
                    ...
                    if (italicText) {
                      yPos = addText(italicText, margin + indent, yPos, { fontSize: 10 });
                    }
                    doc.setFont('helvetica', 'normal');
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      yPos = addText('Main Agreement content could not be loaded.', margin, yPos, { fontSize: 10 });
      yPos += 5;
    }
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('Annex A - Selected Services', pageWidth / 2, yPos, { align: 'center' });
    yPos += 10;
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    yPos = addText('to the Standard Ground Handling Agreement (SGHA) of January 2023', margin, yPos, { fontSize: 10 });
    yPos += 6;
    ...
    doc.setFont('helvetica', 'normal');
    yPos = addText('hereinafter referred to as the \'Carrier\'', margin, yPos, { fontSize: 10 });
    yPos += 6;
    
    // "and:" and handling company name on same line (dynamic)
    const annexLine3Text = 'and:';
    const annexHandlingCompanyName = clientDetails?.name || submission.client_name || 'INDOTHAI KOLKATA PRIVATE LIMITED';
    ...
    doc.setFont('helvetica', 'normal');
    yPos = addText('the Carrier and/or the Handling Company may hereinafter be referred to as the "Party(ies)" effective from:', margin, yPos, { fontSize: 10 });
    yPos += 6;
    ...
    if (submission.checkbox_selections) {
      ...
      Object.keys(submission.checkbox_selections)
        .filter(mainKey => mainKey !== 'serviceTypes' && /^\d+$/.test(mainKey))
        ...
        .forEach((mainSectionNum) => {
          ...
          Object.keys(mainSection)
            ...
            .forEach((sectionKey) => {
              ...
              checkPageBreak(30);
              const sectionTitle = finalSectionTitles[sectionKey] || `Section ${sectionKey}`;
              doc.setFontSize(11);
              doc.setFont('helvetica', 'bold');
              ...
              items.forEach(item => {
                const subItemMatchLetter = item.match(/^(\d+\.\d+\.\d+)\.([a-z]+)$/);
                ...
                const subItemMatchNumeric = item.match(/^(\d+\.\d+\.\d+)\.(\d+)$/);
              ...
              mainItems.forEach((itemKey) => {
                ...
                let fullItemKey = itemKey.includes('.') && itemKey.split('.').length >= 3 
                  ? itemKey 
                  : `${sectionKey}.${itemKey}`;
                ...
                if (!itemText) {
                  itemText = getItemText(normalizeKeyForTemplate(itemKey, sectionKey));
                }
                
                // Debug logging
                if (!itemText) {
                  console.warn(`[PDF] No text found for item: ${itemKey} (fullKey: ${fullItemKey})`, {
                ...
                if (subItems.length > 0) {
                  subItems.sort().forEach((subItem) => {
                    ...
                    let fullSubItemKey = subItem.includes('.') && subItem.split('.').length >= 4
                      ? subItem
                      : `${fullItemKey}.${subItem.split('.').pop()}`;
                    fullSubItemKey = normalizeKeyForTemplate(fullSubItemKey, sectionKey);
                    
                    let subItemText = getItemText(fullSubItemKey);
                    // Fallback: try with parent + sub suffix
                    if (!subItemText) {
                      subItemText = getItemText(normalizeKeyForTemplate(`${fullItemKey}.${subItem.split('.').pop()}`, sectionKey));
                    }
                    // Fallback: try direct lookup (normalized)
                    if (!subItemText) {
                      subItemText = getItemText(normalizeKeyForTemplate(subItem, sectionKey));
                    }
                    
                    const subItemLine = `  - ${subItem}${subItemText ? ': ' + subItemText : ''}`;
                    yPos = addText(subItemLine, margin + 10, yPos, { fontSize: 9 });
                  });
                }
              });
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('Annex B - Additional Information', pageWidth / 2, yPos, { align: 'center' });
    yPos += 10;
    ...
    try {
      ...
      const templateVariables = variablesResponse.data?.data?.templateVariables || [];

      // [PDF Annex B] Debug: log variables available for tables
      console.log('[PDF Annex B] Variables keys:', Object.keys(variables));
      console.log('[PDF Annex B] aircraft_options present:', !!variables.aircraft_options, typeof variables.aircraft_options, variables.aircraft_options ? (typeof variables.aircraft_options === 'string' ? variables.aircraft_options.substring(0, 80) : JSON.stringify(variables.aircraft_options).substring(0, 80)) : '');
      console.log('[PDF Annex B] additional_charges present:', !!variables.additional_charges, typeof variables.additional_charges, variables.additional_charges ? (typeof variables.additional_charges === 'string' ? variables.additional_charges.substring(0, 80) : JSON.stringify(variables.additional_charges).substring(0, 80)) : '');
      ...
      try {
        const annexBTemplateRes = await api.get(
          `/sgha_template_content/get/${submission.agreement_year || 2025}/Annex B/Section Template`
        );
        if (annexBTemplateRes.data?.data?.content) {
          const content = annexBTemplateRes.data.data.content;
          const parsedContent = typeof content === 'string' ? JSON.parse(content) : content;
          console.log('[PDF Annex B] Template field count:', parsedContent.length, 'field types:', parsedContent.map((f) => f && f.type).filter(Boolean).slice(0, 20));
          const editors = parsedContent.filter((f) => f && f.type === 'editor');
          editors.forEach((ed, i) => {
            const html = (ed.value ?? ed.content ?? '').substring(0, 400);
            console.log(`[PDF Annex B] Editor ${i} length=${(ed.value || ed.content || '').length} contains {{ =${(ed.value || ed.content || '').includes('{{')} snippet=`, html);
          });
          const sections = [];
          let currentArticle = null;
          let currentSection = null;
          ...
          parsedContent.forEach((field, index) => {
            if (field.type === 'heading_no') {
              ...
              if (sectionNum.includes('.') && currentMainSection) {
                ...
                if (nextF && nextF.type === 'subheading') {
                  currentSection = { sectionNumber: sectionNum, sectionTitle: nextF.value ?? '', content: null };
            ...
            if (field.type === 'subheading_no' && currentMainSection) {
              ...
              if (headingNo.includes('.')) {
                const nextF = parsedContent[index + 1];
                const sectionTitle = (nextF && nextF.type === 'subheading') ? (nextF.value ?? '') : '';
                currentSection = { sectionNumber: headingNo, sectionTitle, content: null };
                if (currentArticle) currentArticle.sections.push(currentSection);
                return;
              }
            }
            if (field.type === 'editor' && currentArticle) {
              ...
              if (currentSection) currentSection.content = html;
              else currentArticle.sections.push({ sectionNumber: null, sectionTitle: null, content: html });
            }
          });
          ...
          sections.forEach((art) => {
            art.sections.forEach((sec) => {
              ...
              const hasPlaceholders = typeof content === 'string' && content.includes('{{');
              const hasAircraft = typeof content === 'string' && content.includes('aircraft');
              const hasAdditional = typeof content === 'string' && content.includes('additional');
              console.log(`[PDF Annex B] Section ${art.articleNumber}.${sec.sectionNumber || '?'} contentLength=${content.length} has{{=${hasPlaceholders} hasAircraft=${hasAircraft} hasAdditional=${hasAdditional}`);
              if (content.length > 0 && content.length < 500) {
                console.log('[PDF Annex B] Full content:', content);
              } else if (content.length >= 500) {
                console.log('[PDF Annex B] Content snippet (first 500):', content.substring(0, 500));
              }
            });
          });
          ...
          sections.forEach((article) => {
            ...
            doc.text(articleTitleText, pageWidth / 2, yPos, { align: 'center' });
            yPos += 10;
            ...
            article.sections.forEach((section) => {
              ...
              if (section.content) {
                ...
                console.log(`[PDF Annex B] Section ${section.sectionNumber || 'n/a'} parts count=${parts.length}`, parts.map((p) => ({ isTable: p.isTable, tableType: p.tableType, variableName: p.variableName, contentLen: (p.content || '').length })));
                doc.setFontSize(10);
                doc.setFont('helvetica', 'normal');
                parts.forEach((part, partIndex) => {
                  ...
                  if (part.tableType === 'annex_a_selection') {
                    console.log('[PDF Annex B] Rendering annex_a_selection text');
                    yPos = addText('(See Annex A - Selected Services above)', margin, yPos, { fontSize: 10 });
                    yPos += 5;
                    return;
                  }
                  if (part.tableType === 'aircraft_options') {
                    console.log('[PDF Annex B] Rendering aircraft_options table, data=', variables.aircraft_options != null);
                    try {
                      ...
                      if (data == null) {
                        console.log('[PDF Annex B] aircraft_options is null/undefined, drawing empty table structure');
                        checkPageBreak(25);
                        yPos = drawSimpleTable(
                          margin,
                          yPos,
                          ['Aircraft Company', 'Region', 'MOTW', 'Limit Per Incident'],
                          [],
                          colWidthsAircraft,
                      ...
                      const rows = Array.isArray(data)
                        ? data.map((a) => [a.Company_name || '-', a.Flight_type || '-', a.MTOW || '-', a.Limit_per_incident || '-'])
                        : [[data.Company_name || '-', data.Flight_type || '-', data.MTOW || '-', data.Limit_per_incident || '-']];
                      if (rows.length === 0) {
                        console.log('[PDF Annex B] aircraft_options rows empty, drawing empty table');
                        checkPageBreak(25);
                        yPos = drawSimpleTable(
                          margin,
                          yPos,
                        ...
                        console.log('[PDF Annex B] drawSimpleTable aircraft_options rows=', rows.length, rows);
                        checkPageBreak(40);
                        yPos = drawSimpleTable(
                          margin,
                          yPos,
                      ...
                      console.error('[PDF Annex B] Failed to render aircraft_options table, falling back to text:', e);
                      yPos = addText(String(variables.aircraft_options || 'N/A'), margin, yPos, { fontSize: 10 });
                      yPos += 5;
                    }
                    return;
                  }
                  if (part.tableType === 'additional_charges') {
                    console.log('[PDF Annex B] Rendering additional_charges table, data=', variables.additional_charges != null);
                    try {
                      let data = variables.additional_charges;
                      // Even if no variable data was saved, still render an empty table so structure matches Annex B UI.
                      if (data == null) {
                        console.log('[PDF Annex B] additional_charges is null/undefined, drawing empty table structure');
                        checkPageBreak(25);
                        yPos = drawSimpleTable(
                          margin,
                          yPos,
                          ['Serial No.', 'Service', 'Applicable for', 'Unit of Measure'],
                          [],
                          colWidthsCharges,
                          { fontSize: 9, rowHeight: 7 }
                        );
                      ...
                      data = typeof data === 'string' ? JSON.parse(data) : data;
                      const arr = Array.isArray(data) ? data : (data && data.selected ? data.selected : []);
                      ...
                      if (rows.length === 0) {
                        console.log('[PDF Annex B] additional_charges rows empty, drawing empty table');
                        checkPageBreak(25);
                        yPos = drawSimpleTable(
                          margin,
                          yPos,
                        ...
                        console.log('[PDF Annex B] drawSimpleTable additional_charges rows=', rows.length);
                      ...
                      console.error('[PDF Annex B] Failed to render additional_charges table, falling back to text:', e);
                      yPos = addText(String(variables.additional_charges || 'N/A'), margin, yPos, { fontSize: 10 });
                      yPos += 5;
                    }
                    return;
                  }
                  if (part.tableType === 'variable' && part.variableName) {
                    ...
                    const displayName = part.variableName.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
                    doc.setFont('helvetica', 'bold');
                    doc.text(`${displayName}:`, margin, yPos);
                    doc.setFont('helvetica', 'normal');
                    const valueStr = val === undefined || val === null ? 'N/A' : (typeof val === 'object' ? JSON.stringify(val) : String(val));
                    yPos += 4;
        ...
        console.warn('Annex B template not loaded, falling back to variables list:', annexBErr);
      }

      if (variablesResponse.data?.data) {
        // Display custom variables as fallback/supplement (excluding reserved ones)
        const reservedVariables = ['annex_a_selection', 'aircraft_options', 'additional_charges'];
        const customVars = templateVariables.filter(v => !reservedVariables.includes(v));
        if (customVars.length > 0) {
          ...
          customVars.forEach((varName) => {
            ...
            doc.text(`${displayName}:`, margin, yPos);
            doc.setFont('helvetica', 'normal');
            const valueStr = typeof varValue === 'object' ? JSON.stringify(varValue) : String(varValue || 'N/A');
            yPos = addText(valueStr, margin + 40, yPos, { fontSize: 10, maxWidth: maxWidth - 40 });
            yPos += 5;
          });
          yPos += 5;
        }
        ...
        if (variables.aircraft_options) {
          ...
          try {
            const aircraftData = typeof variables.aircraft_options === 'string' 
            ...
            doc.setFontSize(9);
            doc.setFont('helvetica', 'normal');
            yPos = addText(String(variables.aircraft_options), margin + 5, yPos, { fontSize: 9 });
          }
          yPos += 5;
        }
        ...
        if (variables.additional_charges) {
          ...
          try {
            const chargesData = typeof variables.additional_charges === 'string' 
            ...
            if (Array.isArray(chargesData) && chargesData.length > 0) {
              ...
              chargesData.forEach((charge) => {
                checkPageBreak(10);
                const name = charge.Service_name || charge.charge_name || charge.name || 'N/A';
                const detail = charge.unit_or_measure || charge.Charge_type
                  ? [charge.Charge_type, charge.unit_or_measure].filter(Boolean).join(' · ')
            ...
            doc.setFontSize(9);
            doc.setFont('helvetica', 'normal');
            yPos = addText(String(variables.additional_charges), margin + 5, yPos, { fontSize: 9 });
          }
          yPos += 5;
        }
      }
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      yPos = addText('Annex B data could not be loaded.', margin, yPos, { fontSize: 10 });
    }
