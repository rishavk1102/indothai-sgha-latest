...
const Sgha_reportsummary = () => {
  ...
  const openHistoryDialog = async (submission) => {
    ...
    try {
      const res = await api.get(
        `/api/client/annex-a-submissions/${submission.submission_id}/history`,
      );
  ...
  const expandedSubmission = useMemo(
    () =>
      expandedRow
        ? submissions.find((s) => s.submission_id === expandedRow)
        : null,
    [expandedRow, submissions],
  );
  const expandedAgreementYear =
    expandedSubmission?.agreement_year >= 2000 &&
    expandedSubmission?.agreement_year <= 2100
  ...
  useEffect(() => {
    ...
    const filtered = submissions.filter((submission) => {
      ...
      return (
        ...
        (detailsSummary !== "—" &&
          detailsSummary.toLowerCase().includes(searchLower))
      );
    });
  ...
  useEffect(() => {
    ...
    const displayStatus =
      selectedSubmission.status === "In Progress"
        ? "In Progress"
        : selectedSubmission.client_status || "Pending";
    const currentStatusOption = statusOptions.find(
      (option) => option.code === displayStatus,
    );
    ...
    const fetchDialogVariables = async () => {
      try {
        ...
        console.log("[ReportSummary Status Dialog] Variables loaded", {
          ...
          templateVariablesCount: (response.data?.data?.templateVariables || [])
            .length,
        });
        if (response.data?.data) {
        ...
        console.error(
          "[ReportSummary Status Dialog] Error fetching variables:",
          error,
        );
        setDialogVariables({});
        setDialogTemplateVariables([]);
  ...
  const handleStatusUpdate = async () => {
    ...
    if (dialogTemplateVariables.length > 0) {
      ...
      console.log(
        "[ReportSummary Status Update] Company's Pricing validation",
        { emptyOrNa, wouldBlock: emptyOrNa.length > 0 },
      );
      ...
      console.log(
        "[ReportSummary Status Update] No template variables – skipping Company's Pricing validation",
      );
    }
                        ...
                        <div className="d-flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                console.log(
                                  "[ReportSummary] Preview PDF clicked",
                                  {
  ...
  return (
    <>
      ...
      <TabView
        className="mx-0"
        activeIndex={["Pending", "In Progress", "Approved", "Rejected"].indexOf(
          activeStatus,
        )}
        onTabChange={(e) => {
        ...
        {historyLoading ? (
          ...
          <div className="text-center py-4 text-muted">
            ...
            <p className="mt-2 mb-0">
              No edits yet. The agreement has not been modified after
              submission.
            </p>
          </div>
        ) : (
              ...
              >
                <div className="d-flex justify-content-between align-items-start mb-2">
                  ...
                  <small className="text-muted">
                    {formatHistoryDate(edit.created_at)}
                  </small>
                </div>
                {edit.edit_note && (
                  <p className="small mb-2 text-secondary">{edit.edit_note}</p>
                )}
