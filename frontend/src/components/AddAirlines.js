...
const AddAirlines = ({ onClose, page_name }) => {
    ...
    const handleSubmit = async () => {
        try {
            setLoading(true);
            const response = await api.post(`/airlines/add_airline/${page_name}`, formData);
            if (response.status === 201) {
                onClose(); // close dialog or reset form
            }
        } catch (error) {
    ...
    return (
        <>
            <Row className="mx-0 mt-3 addClient">
                <h5 className="py-4">Add New Airline</h5>
                <Row className="mx-0 mt-3">
                    ...
                    <Col md={6} lg={12} className="text-end p-0 mt-3">
                        ...
                        <Button label="Add Airline" className="py-2 border-0" severity="help" onClick={handleSubmit} />
                    </Col>
                </Row>
            </Row>
        </>
